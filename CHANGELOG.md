...
- **卡片刷新按钮不再被旧的启用选择挡住新模型**：`enabledModelIds` 是**针对当时那份目录**做的选择/过滤器。Trae 新增模型后，旧选择里没有它的 id，原样保留过滤器会导致「刷新了但依然看不到新模型」。现在刷新时把实时目录里**尚未出现在选择中**的模型一并纳入（与 Host 对空选择「全部启用」的既有约定一致）；刷新前被用户主动关闭的模型仍在选择内，因此仍保持关闭。

### Tests

- `tests/settings-integration.spec.ts` 新增 3 项集成回归测试（基于真实插件注册路径，非单元桩）：离线启动仍服务存档快照（兜底仍然有效）、恢复网络后任一设置变更即用实时数据替换陈旧快照（去掉修复后该测试确实失败）、后台重发现失败时保持当前目录不变。新增 `networkSwitch()` 辅助函数——`fetch` 必须在插件加载**之前**替换，因为 `TraeSoloClient` / `TraeSoloRemoteCatalogClient` 在构造时捕获 `fetch`。
